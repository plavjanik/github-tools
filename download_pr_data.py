...
parser.add_argument(
    "repo_owner", help="the owner of a GitHub repo. For 'https://github.com/zowe/api-layer' would be 'zowe'"
)
parser.add_argument(
    "repo_name",
    help="the name of a GitHub repo of the above owner. For 'https://github.com/zowe/api-layer' would be 'api-layer'"
)
parser.add_argument("-n", "--num-prs", type=int,
                    help="the total number of PRs to download from the repo", default=1000)
parser.add_argument("--prs-per-batch", type=int,
                    help="the number of PRs to download per request", default=100)
...
if not API_TOKEN_KEY in os.environ:
    print(
        f"There must be a '{API_TOKEN_KEY}' environment variable defined", file=sys.stderr)
    exit(1)
